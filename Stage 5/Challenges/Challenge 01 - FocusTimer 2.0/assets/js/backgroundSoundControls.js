...
export default function BackgroundSoundControls({
  ...
  function toggleSyleButton(buttonClicked) {
    let buttonIconSvg = document.querySelector(
      `#${buttonClicked.id} > svg > path`
    )
    ...
    if (currentButtonActived == buttonClicked) {
      ...
      if (currentButtonActived) {
        ...
        let currentButtonIconSvgActived = document.querySelector(
          `#${currentButtonActived.id} > svg > path`
        )
